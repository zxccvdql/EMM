...
class AdaTTSp(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = EmbeddingLayer(input_dim)
        layer_input_dim = sum([fea.embed_dim for fea in input_dim])
        ...
        for expert_out_dim in expert_out_dims:
            self.experts.append(
                torch.nn.ModuleList(
                    [
                        MLP(layer_input_dim, output_layer=True, dims=expert_out_dim)
    ...
    def forward(
        ...
        embed_x = self.embedding(inputs, self.features, squeeze_dim=True)
        for layer_i in range(self.num_extraction_layers):
            ...
            out_list = []
            for task_index in range(inputs.shape[1]):
                task_out = inputs[:, task_index, :]
                task_out = torch.sigmoid(task_out)
                out_list.append(task_out)
